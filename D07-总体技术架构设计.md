...
| 存储 | 用途 | 架构边界 |
|------|------|----------|
| **MySQL `smart_city`** | 用户/机构/权限、供需、资产登记、审计、指标配置、审批扩展 | Flyway 管理；**业务主库** |
| **OpenMetadata 库** | OM 元数据、质量、目录本体 | 仅经 OM API；业务库存机构映射表 |
...
人口、法人等业务采用**五区 + 七维度**逻辑架构（基线 5.7）；物理上落在 `smart_city` 主题表与交换结果表，V1.1 不展开全量 ER，DDL 在详细设计阶段补充。
...
flowchart LR
  ...
  subgraph exchange [交换_L1.2]
    ESB[AEAI_ESB]
  end
  subgraph analyze [分析_L3]
    Theme[主题库]
    BI[DataEase]
  end
  Src --> Coll
