...
| 6 | **手工部署** | 政务云 VM 手工安装中间件，不使用 Docker / K8s |
...
与 [`D11-统一门户技术方案.md`](D11-统一门户技术方案.md) V0.6 一致；工程目录 `platform-web/`。
...
引用开源框架选型评估 §六，政务云 **手工安装**，**无 Docker**：

| VM | 规格 | 组件 |
|----|------|------|
| **VM1** | 8C / 16GB / 200GB | Nginx、platform-backend、MySQL（或独立 DB）、AEAI ESB、OpenMetadata、DolphinScheduler、Kettle、Redis（备） |
| **VM2** | 4C / 10GB / 500GB | Elasticsearch、SeaweedFS、Canal、MongoDB、DataEase、**Redis（主）** |
| **NAS** | 共享存储 | 文档存储、备份归档、审计只读副本 |

MySQL 可部署在 VM1 或独立数据库 VM；默认与 VM1 同机或内网专机，由运维确认。

### 6.2 VM 组件级内存分配

| VM | 内存分配（GB） | 说明 |
|----|----------------|------|
| **VM1** | Nginx 0.5 + Spring Boot 4 + MySQL 4 + OM 3 + DS 1.5 + Kettle 1.5 + Redis备 0.5 + OS 1.5 | 合计约 16GB |
| **VM2** | ES 2.5 + SeaweedFS 1 + Canal 0.5 + MongoDB 1 + Redis主 0.5 + DataEase 2.5 + OS 1.5 | 合计约 10GB |

> 总内存约 21GB，与框架选型评估终版一致。
...
| 静态资源 | Nginx 托管 `platform-web` 构建产物 |
...
- **资源中心管理**：大数据平台资源中心（M130～M138）负责主题库的库区、分区、备份恢复与存储管理，不重复实现治理加工。
