...
function openEdit(row: IndicatorDomainRow) {
  if (row.hasIndicators) {
    ElMessage.warning('该指标域下已有指标，不能修改')
    return
  }
...
async function removeRow(row: IndicatorDomainRow) {
  if (row.hasIndicators) {
    ElMessage.warning('该指标域下已有指标，不能删除')
    return
  }
...
<template>
  <div v-loading="loading" class="ind-domain-panel">
    ...
    <el-table class="portal-table" :data="rows" stripe size="small" empty-text="暂无数据">
      ...
      <el-table-column label="操作" width="140" fixed="right">
        <template #default="{ row }">
          <el-tooltip
            v-if="row.hasIndicators"
            content="该指标域下已有指标，不能修改或删除"
            placement="top"
          >
            <span class="ind-domain-ops-locked">
              <el-button link type="primary" disabled>编辑</el-button>
              <el-button link type="primary" disabled>删除</el-button>
            </span>
          </el-tooltip>
          <template v-else>
    ...
    <el-dialog
      ...
      :title="editingId == null ? '新增指标域' : '编辑指标域'"
      width="520px"
      destroy-on-close
    >
