-- V219: 四类业务支撑系统指标域/组/任务 owner_domain_code 对齐
-- population|legal|macro|key，便于各 Hub 指标库只查本域数据
...
-- 按名称/库名纠正归属
UPDATE ana_indicator_domain
SET owner_domain_code = 'population', updated_at = NOW()
WHERE status = 'ACTIVE'
  AND (
    domain_db_name = 'ind_population_big_data_support_system'
    OR domain_name = '人口大数据支撑系统'
  );

UPDATE ana_indicator_domain
SET owner_domain_code = 'legal', updated_at = NOW()
WHERE status = 'ACTIVE'
  AND (
    domain_db_name = 'ind_legal_entity_big_data_support_system'
    OR domain_name = '法人大数据支撑系统'
  );

UPDATE ana_indicator_domain
SET owner_domain_code = 'macro', updated_at = NOW()
WHERE status = 'ACTIVE'
  AND (
    domain_db_name IN ('ind_cde', 'ind_macro_economy_big_data_support_system')
    OR domain_name LIKE '%宏观经济%'
  );

UPDATE ana_indicator_domain
SET owner_domain_code = 'key', updated_at = NOW()
WHERE status = 'ACTIVE'
  AND (
    domain_db_name IN ('ind_efg', 'ind_key_domain_demo_support_system')
    OR domain_name LIKE '%重点领域%'
  );
...
WHERE d.status = 'ACTIVE' AND d.owner_domain_code = 'population'
  ...
  AND NOT EXISTS (
    SELECT 1 FROM ana_indicator_group g WHERE g.indicator_domain_id = d.id AND g.status <> 'INACTIVE'
  )
