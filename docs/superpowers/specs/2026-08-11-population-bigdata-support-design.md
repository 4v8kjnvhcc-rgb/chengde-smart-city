...
每区均标明：**逻辑要求** vs **现网形态（挂载 + 深链）**。七维度：定位 / 数据模型 / 加工处理 / 存储周期 / 数据来源 / 使用者 / 更新频度。人口 Hub 已按本表写入各区设计卡文案（见 `AnalyticsDomainHubView.vue`）。
...
| 分期 | 内容 |
|------|------|
| 本期 A | 本规格 + repair 回链 — **已完成** |
| 后续 B | 主题表/校核台账/接口骨架/自研结果页 — **已完成（LEDGER）** |
| 后续 C | Hub 演示骨架（双重授权深链、存储分区摘要、批量台账、采集源清单）— **已完成**；部门真源、物理 ALTER、生产 ESB/前置库、DWS 数据权强制 — **仍依赖外部系统** |
