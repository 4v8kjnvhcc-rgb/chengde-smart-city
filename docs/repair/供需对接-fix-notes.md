...
5. 提供方同意 →「已挂载」（选门户目录，状态 `CATALOG_MOUNTED`）→ 需求部门「办结」→ 供给查看。
6. 可手工把 `catalog_mount_deadline` / `response_deadline` 改到过去验证超时督办。

## 2026.8.8 问题修复
1. 附件：`POST /exchange/supply/attachments/upload` + 表单 el-upload。
2. 新建需求先选 STRUCTURED / UNSTRUCTURED。
3. 需求分析选中需求后默认加载分发部门门户目录。
4. dataItems 规范化回显；提交前冲刷未点「新增」的输入。
5. 「已挂载」弹窗选目录；办结改由数据需求管理操作。
6. 结构/非结构类型选择并入「填写需求信息」表单，去掉先选弹窗。
7. 需求分析默认履约路径 `NEED_COLLECT`（未在中台·需归集补数），去掉「共享」下拉。
8. 部门管理员组织隔离：需求管理按 requesterOrg；需求确认按 assigneeOrg；供给查看「我的需求/我的供给」双 TAB；清单中心按本部门过滤。`GET /demands` 对非平台用户仅返回相关组织记录。
